...
function renderTradeButtons(signal) {
  const krMatch = signal.ticker.match(/^(\d{6})\.(KS|KQ)$/i);
  // 접미사 없는 순수 알파벳 티커면 미국 주식으로 간주 (예: AAPL, TSLA)
  const isUsTicker = /^[A-Z.]{1,6}$/.test(signal.ticker) && !krMatch;

  if (krMatch) {
    const code = krMatch[1];
    return `
      <div class="trade-buttons">
        <button class="btn-trade btn-buy" data-market="domestic" data-code="${code}" data-name="${signal.label}" data-price="${signal.currentPrice}" data-side="buy">매수</button>
        <button class="btn-trade btn-sell" data-market="domestic" data-code="${code}" data-name="${signal.label}" data-price="${signal.currentPrice}" data-side="sell">매도</button>
      </div>`;
  }
  if (isUsTicker) {
    return `
      <div class="trade-buttons">
        <button class="btn-trade btn-buy" data-market="overseas" data-code="${signal.ticker}" data-name="${signal.label}" data-price="${signal.currentPrice}" data-side="buy">매수 (미국)</button>
        <button class="btn-trade btn-sell" data-market="overseas" data-code="${signal.ticker}" data-name="${signal.label}" data-price="${signal.currentPrice}" data-side="sell">매도 (미국)</button>
      </div>`;
  }
  return '';
}
...
const accountEnvBadge = document.getElementById('accountEnvBadge');

function renderHoldingsList(holdings, currencyLabel) {
  if (!holdings || holdings.length === 0) {
    return '<div style="color:var(--ink-dim);font-size:12px;padding:12px 0;">보유 종목 없음</div>';
  }
  return holdings.map((h) => {
    const cls = h.profitLossPct >= 0 ? 'change-up' : 'change-down';
    return `
      <div class="holding-row">
        <div>
          <div class="holding-name">${h.name}</div>
          <div class="holding-meta">${h.quantity}주 · 평단 ${h.avgPrice.toLocaleString()}${currencyLabel}</div>
        </div>
        <div style="text-align:right;">
          <div>${h.evalAmount.toLocaleString()}${currencyLabel}</div>
          <div class="${cls}">${h.profitLossPct >= 0 ? '+' : ''}${h.profitLossPct}%</div>
        </div>
      </div>`;
  }).join('');
}
...
async function loadAccount() {
  try {
    ...
    const data = await res.json();
    const { domestic, overseas } = data;

    if (domestic?.error && overseas?.error) {
      accountContent.innerHTML = `<div class="account-notice">계좌 연동 안 됨: ${domestic.error}</div>`;
      return;
    }

    const env = domestic?.env || overseas?.env;
    accountEnvBadge.textContent = env === 'real' ? '실전투자' : '모의투자';
    accountEnvBadge.className = env === 'real' ? 'env-badge env-real' : 'env-badge env-mock';

    let html = '';

    if (domestic?.error) {
      html += `<div class="account-notice">국내 계좌: ${domestic.error}</div>`;
    } else {
      html += `
        <div class="account-subheading">국내주식</div>
        <div class="account-summary">
          <div><span class="account-label">예수금</span><span>${domestic.cash.toLocaleString()}원</span></div>
          <div><span class="account-label">주문가능현금</span><span>${domestic.orderableCash.toLocaleString()}원</span></div>
          <div><span class="account-label">총 평가금액</span><span>${domestic.totalEvalAmount.toLocaleString()}원</span></div>
          <div><span class="account-label">총 평가손익</span><span class="${domestic.totalProfitLoss >= 0 ? 'change-up' : 'change-down'}">${domestic.totalProfitLoss.toLocaleString()}원</span></div>
        </div>
        <div class="holdings-list">${renderHoldingsList(domestic.holdings, '원')}</div>`;
    }

    if (overseas?.error) {
      html += `<div class="account-notice" style="margin-top:16px;">해외 계좌: ${overseas.error}</div>`;
    } else {
      html += `
        <div class="account-subheading" style="margin-top:20px;">해외주식 (${overseas.exchange})</div>
        <div class="account-summary">
          <div><span class="account-label">총 평가손익</span><span class="${overseas.totalEvalAmountUsd >= 0 ? 'change-up' : 'change-down'}">$${overseas.totalEvalAmountUsd.toLocaleString()}</span></div>
        </div>
        <div class="holdings-list">${renderHoldingsList(overseas.holdings, '달러')}</div>`;
    }

    accountContent.innerHTML = html;
  } catch (err) {
    accountContent.innerHTML = `<div class="account-notice">계좌 정보를 불러오지 못했습니다: ${err.message}</div>`;
  }
}
loadAccount();

// --- 주문 확인 모달 (2단계: 버튼 클릭 -> 확인창 -> 사용자가 직접 확인해야 실행) ---
...
function openOrderModal({ market, code, name, price, side }) {
  pendingOrder = { market, code, name, price: Number(price), side };
  orderModalTitle.textContent = side === 'buy' ? `${name} 매수 주문` : `${name} 매도 주문`;
  orderModalResult.innerHTML = '';

  if (market === 'overseas') {
    // 해외주식은 지정가만 지원 (시장가 TR_ID 미검증이라 안전하게 제외)
    orderModalBody.innerHTML = `
      <div class="order-field">
        <label>종목코드 (거래소: NASD)</label>
        <div>${code}</div>
      </div>
      <div class="order-field">
        <label>주문 수량</label>
      ...
      <div class="order-field">
        <label>지정가 (USD) — 해외주식은 지정가만 지원</label>
        <input type="number" id="orderLimitPrice" value="${Number(price).toFixed(2)}" step="0.01" />
      </div>
      <p class="order-warning">⚠ 확인 버튼을 누르면 실제(또는 모의) 해외계좌로 주문이 즉시 전송됩니다.</p>
    `;
  } else {
    orderModalBody.innerHTML = `
      <div class="order-field">
      ...
      <div class="order-field" id="limitPriceField" hidden>
        <label>지정가 (원)</label>
        <input type="number" id="orderLimitPrice" value="${Math.round(price)}" step="1" />
      </div>
      <p class="order-warning">⚠ 확인 버튼을 누르면 실제(또는 모의) 계좌로 주문이 즉시 전송됩니다.</p>
    `;
    document.getElementById('orderType').addEventListener('change', (e) => {
      document.getElementById('limitPriceField').hidden = e.target.value !== 'limit';
    });
  }
...
orderConfirmBtn.addEventListener('click', async () => {
  ...
  const isOverseas = pendingOrder.market === 'overseas';
  const orderType = isOverseas ? 'limit' : document.getElementById('orderType').value;
  const limitPrice = document.getElementById('orderLimitPrice')?.value;

  orderConfirmBtn.disabled = true;
  orderModalResult.innerHTML = '<div style="color:var(--ink-dim);">주문 전송 중…</div>';
  ...
  try {
    ...
    const data = await res.json();
    if (data.error) throw new Error(data.error);
    orderModalResult.innerHTML = `<div class="order-success">✓ ${data.message || '주문 완료'} (주문번호 ${data.orderNo})</div>`;
    setTimeout(() => { closeOrderModal(); loadAccount(); }, 2000);
  } catch (err) {
    orderModalResult.innerHTML = `<div class="card-error">주문 실패: ${err.message}</div>`;
  } finally {
    orderConfirmBtn.disabled = false;
  }
});

// 매수/매도 버튼 클릭 감지 (검색 결과 영역에 이벤트 위임)
